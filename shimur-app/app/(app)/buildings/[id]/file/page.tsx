...
export default async function FilePage({ params }: Props) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href={`/buildings/${id}`} className="text-sm mb-3 inline-block transition-colors" style={{ color: '#8B7355' }}>
          ← חזרה למבנה
        </Link>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {chapters.map((ch) => (
          ...
          >
            ...
            <div className="w-full h-1 rounded-full mt-3" style={{ backgroundColor: '#EDE0CC' }}>
              <div className="h-1 rounded-full" style={{ width: '0%', backgroundColor: '#4A5C45' }} />
            </div>
            <p className="text-xs text-ink-soft mt-2">לא התחיל</p>
          </Link>
        ))}
      </div>
      ...
      <div className="flex gap-3">
        <button
          className="px-6 py-3 text-white font-medium rounded-md text-sm"
