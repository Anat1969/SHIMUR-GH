...
import { ChapterEditor } from '@/components/file/ChapterEditor';
...
const CHAPTERS_SINGLE: Record<string, { letter: string; title: string; description: string; fields: string[] }> = {
  ...
  tet:   { letter: 'ט', title: 'הצהרת מתעד', description: 'הצהרה חוקית חתומה של האדריכל המתעד', fields: ['שם מתעד','מספר ת.ז.','מספר רישיון','תאריך סיור','תאריך חתימה'] },
};
...
const CHAPTERS_COMPLEX: Record<string, { letter: string; title: string; description: string; fields: string[] }> = {
  ...
  het:   { letter: 'ח', title: 'הערכת ערכים', description: 'ניתוח ערכי השימור לכל מבנה במתחם', fields: ['ערך היסטורי-תרבותי','ערך אדריכלי','ערך אורבני-סביבתי','ערך קהילתי-חברתי','המלצה לכל מבנה'] },
  ...
  yod:   { letter: 'י', title: 'הצהרת מתעד', description: 'הצהרה חוקית חתומה של האדריכל המתעד', fields: ['שם מתעד','מספר ת.ז.','מספר רישיון','תאריך סיור','תאריך חתימה'] },
};

interface Props { params: Promise<{ id: string; chapter: string }> }

async function loadSavedContent(cityRegistryId: string, chapterKey: string): Promise<Record<string, string>> {
  try {
    ...
    if (!file) return {};
    const { data: chapter } = await admin.from('chapters').select('content').eq('file_id', file.id).eq('chapter_key', chapterKey).maybeSingle();
    return (chapter?.content as Record<string, string>) ?? {};
  } catch { return {}; }
}
...
export default async function ChapterPage({ params }: Props) {
  ...
  const savedContent = await loadSavedContent(building!.city_registry_id, chapter);

  return (
    <div className="space-y-6">
      {/* Breadcrumb */}
      <div className="flex items-center gap-2 text-sm flex-wrap" style={{ color: '#8B7355' }}>
      ...
      <div className="flex justify-between items-center pt-4 border-t border-stone-light">
        {prevChapter ? (
          <Link href={`/buildings/${id}/file/${prevChapter}`}
        ...
        {nextChapter ? (
          <Link href={`/buildings/${id}/file/${nextChapter}`}
            className="px-4 py-2 rounded-md text-sm font-medium border transition-colors"
            style={{ borderColor: '#C8B89A', color: '#8B7355' }}>
